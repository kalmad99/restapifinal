...
func (ur *UserRepositoryImpl) UpdateUser(hashpass string, u entity.User) error {
	...
	//_, err := ur.conn.Exec("UPDATE users SET name=$1,email=$2,phone=$3, password=$4 WHERE id=$5 AND password <> $4",
	//	u.Name, u.Email, u.Phone, u.Password, u.ID, u.Password)
	//_, err := ur.conn.Exec("UPDATE users SET name=$1,email=$2,phone=$3, password=$4 WHERE id=$5 AND password=$6);",
	//	u.Name, u.Email, u.Phone, u.Password, u.ID, u.Password)
	if err != nil {
...
func (ur *UserRepositoryImpl) ChangePassword(u entity.User) error {
	...
	_, err := ur.conn.Exec("UPDATE users SET password=? WHERE id=?", u.Password, u.ID)
	if err != nil {
