...
func NewCartRepositoryImpl(Conn *sql.DB) *CartRepositoryImpl {
	return &CartRepositoryImpl{conn: Conn}
}

// Categories returns all cateogories from the database
func (cri *CartRepositoryImpl) ItemsinCart() ([]entity.Product, error) {

	rows, err := cri.conn.Query("SELECT * FROM add_to_carts;")
	if err != nil {
	...
	for rows.Next() {
		cart := entity.Cart{}
		err = rows.Scan(&cart.CartID, &cart.PlacedAt, &cart.UserID, &cart.ItemID, &cart.Quantity)
		if err != nil {
			return nil, err
		}
...
func (cri *CartRepositoryImpl) Category(id uint) (entity.Category, error) {
...
func (cri *CartRepositoryImpl) UpdateCategory(c entity.Category) error {
...
func (cri *CartRepositoryImpl) DeleteCategory(id uint) error {
...
func (cri *CartRepositoryImpl) StoreCategory(c entity.Category) (entity.Category, error) {
...
func (cri *CartRepositoryImpl) ItemsInCategory (category entity.Category) ([]entity.Product, error){
